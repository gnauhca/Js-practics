jQuery('#toptb .wp .y a').attr('onclick', '').unbind('click').bind("click", function() {
	fix(this)
});
if (jQuery('html').hasClass('widthauto')) {
	masonryPoster.setWindowWidthColumn('#wp', masonryPoster.options.contentWidth);
}
...
function layout_reset() {
	jQuery("#wall").masonry('reload');
}

function fix(id) {
	if (!jQuery('html').hasClass('widthauto')) {
		masonryPoster.options.contentId = '#wp';
		masonryPoster.options.isFixed = true;
		masonryPoster.setWindowWidthColumn('#wp', masonryPoster.options.contentWidth);

	} else {
		jQuery('#wp').removeAttr('style');
		masonryPoster.options.isFixed = false;
	}
	widthauto(id);
	setTimeout(function() {
		jQuery("#wall").masonry('reload')
...
jQuery(window).bind('scroll', function() {
	if (filternav.length > 0) {
		if (jQuery(document).scrollTop() <= ftop && jQuery('.fixed-nav').hasClass('enable')) {
			jQuery('.fixed-nav').removeClass('enable').css('margin-left', 'auto');
		...
		if (jQuery(document).scrollTop() > ftop) {
			if (jQuery('html').hasClass('widthauto')) {
				jQuery('.fixed-nav').addClass('enable').css('width', jQuery('#wall').width() - 16 + 'px').css('margin-left', (jQuery('#wp').width() - 14) / 2 * (-1) + "px");
			} else {
				jQuery('.fixed-nav').addClass('enable').css('width', '926px').css('margin-left', "-464px");
			}
